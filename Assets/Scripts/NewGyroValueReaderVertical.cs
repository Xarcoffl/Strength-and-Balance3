...
public class NewGyroValueReaderVertical : MonoBehaviour
{
    ...
    private float[] GyroDataArrayv = null;
    ...
    void Start()
    {
        ...
        angleGyro = Mathf.Atan2(accel.y, Mathf.Sqrt(accel.x * accel.x + accel.z * accel.z));// * Mathf.Rad2Deg;
    }
    ...
    private void CollectGyroData()
    {
        ...
        angleGyro += gyroValue.x * dt;// * Mathf.Rad2Deg * dt;
        Vector3 accel = Input.acceleration;
        float angleAccel = Mathf.Atan2(accel.y, Mathf.Sqrt(accel.x * accel.x + accel.z * accel.z)); //* Mathf.Rad2Deg;
    ...
    private void SaveGyroData()
    {
        ...
        GyroDataArrayv = new float[NewDataLength];
        GyroDataArrayv = gyroDataArray.ToArray();
        // Convert the array to JSON
        for (int k = 0; k < GyroDataArrayv.Length; k++)
        {
            Debug.Log(GyroDataArrayv[k]);
        }
        string jsonData = JsonUtility.ToJson(new GyroDataArrayWrapper { gyroData = GyroDataArrayv });
