...
public class GyroScreenGraph : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        PlayerPrefs.SetString("Levelh", formattedTime);
        PlayerPrefs.Save();
    ...
    void Update()
    {
        if (circleRectTransform != null)
        {
            ...
            float xGyro = 2 * Input.gyro.rotationRateUnbiased.y;

            //Vector3 gyroValue = Input.gyro.rotationRateUnbiased; // Gyroscope data in radians per second
            //angleGyro += gyroValue.y * dt;
            Vector3 accel = Input.acceleration;
            float angleAccel = Mathf.Atan2(accel.x, Mathf.Sqrt(accel.y * accel.y + accel.z * accel.z));
    ...
    {
        xMin = -graphContainer.sizeDelta.x / 2;
        xMax = graphContainer.sizeDelta.x / 2;
    }
    ...
    private void MoveCircle(float xGyro)
    {
        ...
        newPosition.x = xGyro * sensitivityvalue;
